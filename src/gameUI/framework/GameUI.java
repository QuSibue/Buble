...
import events.EvNewGame;
import events.EvQuit;
...
public class GameUI {
	...
	JMenu m_menu;
	JLabel m_score;
	JMenuItem m_newGame,m_quit;

	public GameUI(Model m, GameView v, GameController c, Dimension d) {
		m_model = m;
		m_model.setGameUI(this);
		m_view = v;
		m_view.m_game = this;
		m_controller = c;
	...
	private void createWindow(Dimension d) {
		...
		m_score = new JLabel("Score : " + m_model.getGame().getPlateau().getScore());
		m_newGame = new JMenuItem("Nouvelle Partie");
		m_newGame.addActionListener(new EvNewGame(m_model.getGame(),this));
		m_quit = new JMenuItem("Quitter");
		m_quit.addActionListener(new EvQuit());
		
		m_menu.add(m_newGame);
		m_menu.add(m_quit);
		m_menuBar.add(m_menu);
		m_menuBar.add(m_score);
		m_frame.add(m_menuBar, BorderLayout.NORTH);
	...
	private void createTimer() {
		int tick = Options.REPAINT_DELAY; // one millisecond
